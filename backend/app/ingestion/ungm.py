...
NOTE: UNGM does not publish a fully documented public JSON API. The public
notice search (`/Public/Notice/Search`) accepts a POST and returns a JSON
payload of notice rows. Field names below reflect the observed response shape;
`map_record` is deliberately tolerant (multiple candidate keys) and isolated so
it can be unit-tested against a saved sample and adjusted when the live schema
is confirmed.
"""
...
from app.ingestion.base import SourceAdapter, http_post_json
from app.normalize import clean_text, parse_date
from app.schemas import NormalizedOpportunity

logger = logging.getLogger("worktube.ingestion.ungm")
...
def map_record(rec: dict) -> NormalizedOpportunity:
    ...
    ref = _first(rec, "Reference", "ReferenceNo")
