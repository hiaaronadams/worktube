...
def test_dedup_key_prefers_external_id():
    a = _opp(external_id="ABC123")
    assert compute_dedup_key(a) == "sam:ABC123"
...
def test_content_hash_stable_when_unchanged():
    assert compute_content_hash(_opp(summary="same")) == compute_content_hash(
        _opp(summary="same")
    )
